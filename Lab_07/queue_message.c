...
void swap_func(int *digits, int x, int y)
{
    int tmp = digits[x];
    digits[x] = digits[y];
    digits[y] = tmp;
...
int gen_func(int *digits, size_t n)
{
    int j = n - 2;
    ...
    swap_func(digits, j, k);
    int l = j + 1, r = n - 1;
    while (l < r)
    {
        swap_func(digits, l++, r--);
    }
    return 1;
}
...
void *child(void *thd_args)
{
    ...
    qsort(message.digits, 4, sizeof(int), func);

    message.mtype = 101;
    msgsnd(msgId, &message, sizeof(message), 0);
    while (gen_func(message.digits, 4))
    {
        message.mtype = 101;
        msgsnd(msgId, &message, sizeof(message), 0);
    }
...
int main()
{
    ...
    pthread_create(&childThread, NULL, child, (void *)args_var);
